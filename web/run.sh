...
bash -c 'debconf-set-selections <<< "server-5.5 mysql-server/root_password password $server[dbPass]"'
bash -c 'debconf-set-selections <<< "server-5.5 mysql-server/root_password_again password $server[dbPass]"'
...
mkdir ~/ngn-env/logs
cd ~/ngn-env
git clone -b dev https://github.com/majexa/ngn.git
